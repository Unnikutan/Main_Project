<?php
	...
	include '../controls/admin_controls.php';
		...
		$sql_hu = fetch_driver_by_id($conn,$dr_id);
		$res_hu = mysqli_fetch_array($sql_hu);
		$off_id = $res_hu['gb_id'];
?>
				...
				<form method="post" action="complaint_submit.php">
					<div class="row">
						<div class="col-md-5 comp_margin">
							Select Office : 
							<select class="comp_inp" name="gb_name" required>
								<?php
									$sql_gb = fetch_gb_by_id($conn,$off_id);  
									$row_gb=mysqli_fetch_array($sql_gb); 
										?>
										<option value="<?php echo $row_gb['gb_id']?>" selected><?php echo $row_gb['gb_name'] ?></option>
										<?php
								?>
							</select>
						</div>
